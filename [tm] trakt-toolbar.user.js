( function () {
  ...
  if ( url.includes( '/seasons/all' ) ) {
    ...
    if ( url.includes( '/episodes' ) ) {
      episode = true;
      const episodeNumber = url.match( /episodes\/(\d+)$/ )?.[ 1 ]?.padStart( 2, '0' );
  ...
  function createToolbarItem ( targetUrl, iconUrl, displayText = '█  ', toolTip = '' ) {
    const toolbarItem = generateElements( `<a href=${ targetUrl } target=_blank title="${ toolTip }" class=toolbar-item> ${ displayText } </a>` );
    style( toolbarItem, `
      text-wrap: nowrap;
      overflow: hidden;
    `);
    ...
    style( toolbarItem, `
      ...
      font-size: small;
    `);
    if ( iconUrl ) { //! Ublock removes the entire element when this part is added
      //! disable Ublock in order for this to work
      const iconEl = generateElements( `<img class=toolbarIcon src=${ iconUrl }>` );
  ...
  createToolbarItem( // wikis
    `https://www.google.com/search?q=${ query }%20wiki`,
    null, 'Wikis'
  );
  createToolbarItem( // tvtropes
  ...
  createToolbarItem( // reddit discussions
    `https://www.google.com/search?&q=${ query }+discussion%20site:reddit.com`,
    'https://cdn-icons-png.flaticon.com/512/4053/4053291.png', 'Disc'
  );
  createToolbarItem( // reddit
    `https://www.google.com/search?&q=${ query }%20site:reddit.com`,
    'https://cdn-icons-png.flaticon.com/512/725/725298.png', 'Reddit'
  );
  createToolbarItem( // youtube
    `https://www.youtube.com/results?search_query=${ query }%20-reaction%20-trailer%20-review%20-"movie%20clip"`,
    'https://cdn-icons-png.flaticon.com/512/1383/1383260.png', 'Youtube'
  );
  createToolbarItem( // youtube
    `https://www.youtube.com/results?search_query=${ query }+cast`,
    'https://cdn-icons-png.flaticon.com/512/1383/1383260.png', 'Cast'
  );
  createToolbarItem( // leet
    `https://1337x.to/search/${ queryClean }%20720p/1/`,
    'https://www.wizcase.com/wp-content/uploads/2022/10/en-1337x-logo.jpg', 'Leet'
  );
  createToolbarItem( // piratebay
    `https://thepiratebay.org/search.php?q=${ queryClean }%20720p`,
    'https://cdn-icons-png.flaticon.com/512/1119/1119638.png', 'Piratebay'
  );
  createToolbarItem( // ratingraph
    `https://www.google.com/search?btnI=1&q=${ movieShowTitle }%20site:ratingraph.com#${ query }`,
    'https://cdn.ratingraph.com/assets/images/icon-180.png', 'Ratingraph'
  );
