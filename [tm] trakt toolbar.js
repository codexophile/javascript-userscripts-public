query = ''
queryClean = ''
episode = false
season = false
showAll = false

shows = false
seasons = false

if ( location.href.includes( '/seasons/all' ) ) {
  movieShowTitle = location.href.match( /shows\/(.+?)(\/|$)/ )[ 1 ]
  query += `${ movieShowTitle }`
  showAll = true
}
else if ( location.href.includes( '/shows/' ) ) {
  movieShowTitle = location.href.match( /shows\/(.+?)(\/|$)/ )[ 1 ]
  seasonNumber = location.href.match( /seasons\/(\d+?)(\/|$)/ )[ 1 ].padStart( 2, "0" )
  query += `${ movieShowTitle } (s|season) ${ seasonNumber }`
  queryClean = `${ movieShowTitle }%20s${ seasonNumber }`
}
else if ( location.href.includes( '/seasons' ) ) {
  showAll = false
  season = true
  movieShowTitle = location.href.match( /shows\/(.+?)(\d\d\d\d)?(\/|$)/ )[ 1 ]
  seasonNumber = location.href.match( /seasons\/(\d+?)(\/|$)/ )[ 1 ].padStart( 2, "0" )
  query += `${ movieShowTitle } (s|season) ${ seasonNumber }`
  queryClean = `${ movieShowTitle }%20s${ seasonNumber }`
}
if ( location.href.includes( '/episodes' ) ) {
  episode = true
  console.log( location.href )
  episodeNumber = location.href.match( /episodes\/(\d+?)$/ )[ 1 ].padStart( 2, "0" )
  episodeTitle = $( '.main-title' ).text()
  query += ` (e|episode) ${ episodeNumber } ${ episodeTitle }`
  queryClean += `e${ episodeNumber }`
}
if ( location.href.includes( '/movies' ) ) {
  movieShowTitle = location.href.match( /movies\/((\w|\d|-)+)$/ )[ 1 ]
  movieShowTitle = movieShowTitle.replaceAll( '(', '' ).replaceAll( ')', '' )
  query = movieShowTitle
}

if ( location.href.includes( 'shows' ) ) shows = true
if ( location.href.includes( 'seasons' ) ) seasons = true
if ( location.href.includes( 'episode' ) ) episode = true

generateToolbar()

function generateToolbar () {

  query = query.replaceAll( ' ', '%20' )
  query = query.replaceAll( '#', '%20' )
  query = query.replaceAll( '-', '%20' )
  query = query.replaceAll( '%20%20', '%20' )
  if ( !queryClean ) queryClean = query
  imdbHref = `https://www.google.com/search?btnI=1&q=${ query }%20site:imdb.com/title`
  wikiHref = `https://www.google.com/search?btnI=1&q=${ query }%20site:wikipedia.org`
  tvTrHref = `https://www.google.com/search?btnI=1&q=${ query }%20site:tvtropes.org`
  $letterboxdHref = `https://www.google.com/search?btnI=1&q=${ query }%20site:letterboxd.com`
  RedGDisHref = `https://www.google.com/search?&q=${ query }+discussion%20site:reddit.com`
  RedGHref = `https://www.google.com/search?&q=${ query }%20site:reddit.com`
  simklHref = `https://simkl.com/search/?type=movies&q=${ query }`
  rgHref = `https://www.google.com/search?btnI=1&q=${ movieShowTitle }%20site:ratingraph.com`
  leetHref = `https://1337x.to/search/${ queryClean }%20720p/1/`
  tpbHref = `https://thepiratebay.org/search.php?q=${ queryClean }%20720p`
  hrefPlot = `https://www.google.com/search?q=${ query }%20plot`
  ytHref = `https://www.youtube.com/results?search_query=${ query }%20-reaction%20-trailer%20-review%20-"movie%20clip"`
  // leetHref   = `https://1337x.to/search/${query.replaceAll( '(s|season)%20', 's').replaceAll( '%20(e|episode)%20', 'e').replaceAll( episodeTitle.replaceAll( ' ', '%20' ), '' )}%20720p/1/`

  let $toolBar = $( `
    <div id=toolBar>
      <div id=row1></div>
      <div id=row2></div>
      <div id=row3></div>
    </div>` )
  $toolBar.appendTo( document.body )

  imdbUri = 'data:image/png;base64,'
  wikiUri = 'data:image/png;base64,'
  rgUri = 'https://cdn.ratingraph.com/assets/images/icon-180.png'
  tvtrUri = 'data:image/png;base64,'
  UriLeet = 'https://www.wizcase.com/wp-content/uploads/2022/10/en-1337x-logo.jpg'
  UriTPB = 'https://cdn-icons-png.flaticon.com/512/1119/1119638.png'
  rgDisUri = 'https://cdn-icons-png.flaticon.com/512/4053/4053291.png'
  redGUri = 'https://cdn-icons-png.flaticon.com/512/725/725298.png'
  uriPlot = 'https://cdn-icons-png.flaticon.com/512/3336/3336640.png'
  ytUri = 'https://cdn-icons-png.flaticon.com/512/1383/1383260.png'

  if ( seasons && !episode ) {
    $ratingButton = $( '<button> ⭐ </button>' ).appendTo( $toolBar )
    $ratingButton.on( 'click', function () {
      sum = 0
      text = ''
      let $ratings = $( '.fanart > .corner-rating' )
      let ratingIndex = 0
      $ratings.each( function () {
        ratingIndex++
        $this = $( this )
        let ratingText = $this.text()
        text += ` + ${ ratingText }`
        sum += Number( $this.text() )
      } )
      alert( `( ${ text } ) / ${ ratingIndex } = ${ sum / $ratings.length }` )
    } )
  }

  GM_addStyle( `

  #toolBar a {
    margin:  3px;
    color: white;
    text-shadow:
        0.07em 0 black,
        0 0.07em black,
        -0.07em 0 black,
        0 -0.07em black;
  }

  .toolbarIcon {
    width : 32px;
    height: 32px;
  }

  ` )
  $toolBar.css( 'position', 'fixed' ).css( 'background-color', '#ed1c24' )
  $toolBar.css( 'display', 'flex' ).css( `flex-direction`, `column` )
  $toolBar.css( 'right', '30px' ).css( 'top', '150px' )
  $toolBar.children().css( 'padding', '5px' )
  $toolBar.children().css( 'text-align', 'center' )
  $toolBar.find( 'img' ).css( 'display', 'block' )

  let $row1 = $( `#row1` )
  appendItem( $row1, wikiHref, wikiUri )
  appendItem( $row1, tvTrHref, tvtrUri )
  appendItem( $row1, imdbHref, imdbUri )
  appendItem( $row1, RedGDisHref, rgDisUri, undefined, 'Reddit discussions' )
  appendItem( $row1, ytHref, ytUri )

  let $row2 = $( `#row2` )
  appendItem( $row2, leetHref, UriLeet )
  appendItem( $row2, tpbHref, UriTPB )
  appendItem( $row2, RedGHref, redGUri )
  appendItem( $row2, rgHref, rgUri )

  let $row3 = $( `#row3` )
  appendItem( $row3, hrefPlot, uriPlot )
  appendItem( $row3, simklHref,
    'https://play-lh.googleusercontent.com/DliaDatmrt_M8drBtsafddTyhcxN5W3UAcpQRjoq7MViP3iwHBMegVmKIxDAjHrFACQ=w240-h480-rw',
    'SIMKL' )
  appendItem( $row3, $letterboxdHref,
    'https://play-lh.googleusercontent.com/PFcm5Ne2otuXxkCNgql_XtpHjYrlhIGGQRFaz9XLFg2wikmMP5YCv_OsvFe1PLDAvGg',
    'Ltrbxd' )

  //// $simklLink = $( `<a href=${simklHref}  target=_blank > SIMKL</a>` ).appendTo( $toolBar )
  //// $letterboxdLink = $( `<a href=${$letterboxdHref} target=_blank > Letterboxd</a>` ).appendTo( $toolBar )

}

function appendItem ( parent, href, imageUrl, text = '█  ', toolTip = '' ) {
  $( `<a href=${ href } target=_blank title=${ toolTip }> ${ text } </a>` ).
    appendTo( parent ).prepend( `<img class=toolbarIcon src=${ imageUrl }>` )
}