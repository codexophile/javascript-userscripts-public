(async function () {
  ...
  const buttonOriginal = document.getElementsByClassName(
    `SimklTVDetailEpisodeSeriesfakeflexbtn`
  )[0];
  ...
  buttonLeet.onclick = () => {
    window.open(
      `https://1337x.to/search/${showName}%20S${seasonN}E${episodeN}%20720p/1/`
    );
  };
  ...
  function getImdbId() {
    const imdbLinkEl = document.querySelector(
