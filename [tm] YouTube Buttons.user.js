( function () {
    ...
    waitFor( '#collapsibleContent' ).then( ( el ) => {
        ...
        const showSbHorizontalBtn = generateElements( '<button>🎞️</button>', el )
    ...
    async function main () {
        ...
        await waitFor( '#collapsibleContent' )
        const toolbarEl = document.querySelector( '#collapsibleContent' )

        document.querySelectorAll( `.videoPageControl` ).forEach( item => { item.remove() } )
        document.querySelectorAll( `.storyboardControl` ).forEach( item => { item.remove() } )
        calculateWidthAndExpand( toolbarEl )
        ...
        if ( location.href.includes( '/watch?v=' ) ) {
            ...
            const autoPauseChckbx = GM_addElement( toolbarEl, 'input', { type: 'checkbox' } )
