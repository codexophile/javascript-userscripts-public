(async function () {
  ...
  GM_addValueChangeListener("tropesSeen", () => {
    unsavedChanges = true;
  });
  GM_addValueChangeListener("tropesImportant", () => {
    unsavedChanges = true;
  });
  ...
  function test() {
    ...
    console.log(
      tropesIm.filter(
        (currentVal, index, array) => array.indexOf(currentVal) !== index
      )
    );
  ...
  $(`[href*="/Main/"]`).each(function () {
    ...
    if (!location.href.includes("/laconic/")) {
      $tropeLink.attr(
        "href",
        $tropeLink.attr("href").replace("/Main/", "/laconic/")
      );
    ...
    $(`<span class=ant>🐜</span>`)
      .insertAfter($tropeLink)
      .on("click", function () {
        let allToCopy = `\n[href*=${tropeName}],`;
        GM_setClipboard(allToCopy);
      });
    $(`<span class=ant>✖️</span>`)
      .insertAfter($tropeLink)
      .on("click", async () => {
        appendTrope("Seen");
      });
  ...
  async function markAndRefresh() {
    ...
    $(`[href*="/pmwiki/pmwiki.php/"]`).each(function () {
      ...
      if (tropesSeen.includes(tropeName)) mark(this, "#690f08");
      if (tropesImportant.includes(tropeName)) mark(this, "#07540b");
    });
