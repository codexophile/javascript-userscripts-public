(async function () {
  ...
  const selector =
    "div.container.main-container div.graphBody-wrapper div.text-center > p";
  waitForEach(selector, async (locatorEl) => {
    ...
    const estdDailyUsage = getEstdDailyUsage(
      remainingDataAmount,
      remainingDays
    );
  ...
  async function displayEstdDailyUsage(
    remainingDataAmount,
    remainingDays,
    estdDailyUsage
  ) {
  ...
  async function getRemainingDataAmount() {
    const selector =
      "div.container.main-container div.graphBody-wrapper div.text-center > .used-of";
  ...
  function getRemainingTimeToday() {
    ...
    // Set endOfDay to 23:59:59.999 today
  ...
  function getRemainingDays(start, end) {
    ...
    const remainingTimeInDays =
      remainingTimeInMilliseconds / millisecondsInADay;
